...
function loadCoupangPending(){
  ...
  db.ref('users/'+currentUser.uid+'/purchases').once('value').then(function(snap){
    ...
    pending.forEach(function(p){
      var m = findMappedProduct(p.coupangProductName);
      if(!m && typeof findProduct === 'function'){
        var ap = findProductById(p.coupangProductName);
        if(ap) m = true;
      }
      if(m) mappedCount++;
    });
    var batchBtn = mappedCount > 0
      ? '<button onclick="batchStockInAll()" style="width:100%;background:var(--green);color:#fff;border:none;border-radius:10px;padding:12px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;margin-bottom:10px">📦 매칭된 '+mappedCount+'건 일괄 입고처리</button>'
      : '';
    list.innerHTML = batchBtn + pending.map(function(p){
...
function batchStockInAll(){
  if(!currentUser) return;
  if(!confirm('매칭된 모든 구매 건을 일괄 입고처리 할까요?')) return;
  db.ref('users/'+currentUser.uid+'/purchases').once('value').then(function(snap){
    var purchases = snap.val() || [];
    if(!Array.isArray(purchases)) purchases = Object.values(purchases);
    ...
    purchases.forEach(function(p, idx){
      ...
      if(!mapped && typeof findProduct === 'function'){
        var autoProd = findProductById(p.coupangProductName);
        if(autoProd) mapped = {productId: autoProd.id, unitsPerBox: 1};
      }
    ...
    var msg = '✅ '+processed+'건 일괄 입고 완료';
    if(skipped > 0) msg += ' (미매칭 '+skipped+'건 제외)';
    showToast(msg);
    loadCoupangPending();
    renderInv();
  });
